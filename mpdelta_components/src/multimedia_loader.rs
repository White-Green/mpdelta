...
struct FfmpegMultimediaLoader {
    parameter_type: Box<[(String, ParameterType)]>,
    queue: Arc<Queue>,
    ...
    pub fn new(queue: &Arc<Queue>, gpu_memory_allocator: &Arc<GenericMemoryAllocator<FreeListAllocator>>, command_buffer_allocator: &Arc<StandardCommandBufferAllocator>) -> FfmpegMultimediaLoaderClass {
        FfmpegMultimediaLoaderClass {
            processor: Arc::new(FfmpegMultimediaLoader {
                parameter_type: Box::new([("media_file".to_owned(), ParameterType::Binary(()))]),
...
#[async_trait]
impl<K, T> ComponentClass<K, T> for FfmpegMultimediaLoaderClass
where
...
#[async_trait]
impl<K, T> ComponentProcessor<K, T> for FfmpegMultimediaLoader
where
    T: ParameterValueType<Image = ImageType, Audio = AudioType>,
{
...
#[async_trait]
impl<K, T> ComponentProcessorNative<K, T> for FfmpegMultimediaLoader
where
    T: ParameterValueType<Image = ImageType, Audio = AudioType>,
{
