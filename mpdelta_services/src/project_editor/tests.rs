...
async fn pretty_print<K, T>(value: &RootComponentClassHandleOwned<K, T>, key: &TCellOwner<K>) -> String
where
    K: 'static,
    T: ParameterValueType,
{
...
async fn assert_eq_root<K, T>(a: &RootComponentClassHandleOwned<K, T>, b: &RootComponentClassHandleOwned<K, T>, key: &TCellOwner<K>)
where
    K: 'static,
    T: ParameterValueType,
{
    ...
    .await;
    if result.is_none() {
        let a = pretty_print(a, key).await;
        let b = pretty_print(b, key).await;
    ...
    (
        ...
        let components = vec![$({
            ...
            let builder = ::mpdelta_core::component::instance::ComponentInstance::builder(
                ::mpdelta_core::ptr::StaticPointer::<::tokio::sync::RwLock<NoopComponentClass>>::new().map(|c| c as _),
                marker_left,
                marker_right,
                markers,
                ::mpdelta_core::component::processor::ComponentProcessorWrapper::Native(::std::sync::Arc::new(NoopProcessor) as ::std::sync::Arc<dyn ::mpdelta_core::component::processor::ComponentProcessorNativeDyn<$k, $t>>),
            );
...
async fn test_edit_marker_link_length() {
    ...
    root_component_class! {
        ...
        left: left,
        components: [
            { markers: [marker!(mfrac!(0)) => l1, marker!() => r1] },
        ],
        links: [
    ...
    assert_eq_root(&edit_target, &expect, &key!()).await;
}
...
async fn test_delete_component_instance() {
    ...
    root_component_class! {
        ...
        left: left,
        components: [
            { markers: [marker!(mfrac!(0)) => l1, marker!() => r1] }; c1,
            { markers: [marker!(mfrac!(0)) => l2, marker!() => r2] },
        ],
        links: [
            left = mfrac!(1) => l1,
            l1 = mfrac!(1) => l2,
    ...
    root_component_class! {
        ...
        components: [
            { markers: [marker!(mfrac!(0)) => l1, marker!() => r1] },
            { markers: [marker!(mfrac!(0)) => l2, marker!() => r2] }; c2,
        ],
        links: [
            left = mfrac!(1) => l1,
            l1 = mfrac!(1) => l2,
    ...
    root_component_class! {
        ...
        components: [
            { markers: [marker!(mfrac!(0)) => l1, marker!() => r1] },
            { markers: [marker!(mfrac!(0)) => l2, marker!(mfrac!(1)) => r2] }; c2,
            { markers: [marker!(mfrac!(0)) => l3, marker!() => r3] },
        ],
        links: [
            left = mfrac!(1) => l1,
            l1 = mfrac!(1) => l2,
...
async fn test_connect_marker_pins() {
    ...
    root_component_class! {
        ...
        left: left,
        components: [
            { markers: [marker!(mfrac!(0)) => l1, marker!(mfrac!(1)) => r1] },
            { markers: [marker!(mfrac!(0)) => l2, marker!(mfrac!(1)) => r2] },
        ],
        links: [
            left = mfrac!(1) => l1,
            l1 = mfrac!(1) => r1,
